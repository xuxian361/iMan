...
import com.sundy.iman.entity.NearbyPostItemEntity;
...
public class NearbyPostActivity extends BaseActivity {
    ...
    private List<NearbyPostItemEntity> listPost = new ArrayList<>();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_nearby_post);
    ...
    private void initTitle() {
        titleBar.setBackMode(getString(R.string.nearby));
    ...
    private class PostAdapter extends BaseQuickAdapter<NearbyPostItemEntity, BaseViewHolder>
        ...
        public PostAdapter(@LayoutRes int layoutResId, @Nullable List<NearbyPostItemEntity> data) {
            super(layoutResId, data);
            gsyVideoOptionBuilder = new GSYVideoOptionBuilder();
        }

        @Override
        protected void convert(final BaseViewHolder helper, final NearbyPostItemEntity item) {
            try {
                ...
                NearbyPostItemEntity.MemberEntity memberEntity = item.getMembers();
                ...
                ImageHelper.displayPortrait(NearbyPostActivity.this, profileImg, iv_header);

                //来源
                List<NearbyPostItemEntity.CommunityEntity> community_list = item.getCommunity_list();
                if (community_list != null && community_list.size() > 0) {
                    NearbyPostItemEntity.CommunityEntity communityEntity = community_list.get(0);
                    if (communityEntity != null) {
                        String community_name = communityEntity.getName();
                        final String community_id = communityEntity.getId();
                        tv_from.setText(getString(R.string.from) + ": " + community_name);
                ...
                if (attachment != null && attachment.size() > 0) {
                    ...
                    if (size == 1) {
                        ...
                        NearbyPostItemEntity.AttachmentEntity attachmentEntity = attachment.get(0);
...
//                                            GSYVideoManager.instance().setNeedMute(true);
                            ...
                            gsyVideoPlayer.getFullscreenButton().setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    gsyVideoPlayer.startWindowFullscreen(NearbyPostActivity.this, true, true);
                                }
                            });
                        }
                    } else {
                        ...
                        if (size == 2 || size == 4)
                            rv_media.setLayoutManager(new GridLayoutManager(NearbyPostActivity.this, 2));
                        else
                            rv_media.setLayoutManager(new GridLayoutManager(NearbyPostActivity.this, 3));
                ...
                //是否已读
                boolean isRead = PaperUtils.isPostRead("nearby", post_id);
                if (isRead) {
                    view_top.setBackgroundColor(ContextCompat.getColor(NearbyPostActivity.this, R.color.bg_gray));
                    iv_dot.setImageResource(R.mipmap.icon_dot_gray);
                } else {
                    view_top.setBackgroundColor(ContextCompat.getColor(NearbyPostActivity.this, R.color.bg_blue));
                        ...
                        if (PaperUtils.isLogin()) { //已登录
                            ...
                            if (userId.equals(creator_id)) {
                                if (postItemMenuSelfPopup == null) {
                                    postItemMenuSelfPopup = new PostItemMenuSelfPopup(NearbyPostActivity.this);
                                }
                                ...
                                if (postItemMenuPopup == null) {
                                    postItemMenuPopup = new PostItemMenuPopup(NearbyPostActivity.this);
                                }
            ...
            switch (view.getId()) {
                case R.id.rel_item:
                    ...
                    if (itemData != null) {
                        ...
                        PaperUtils.savePostReadID("nearby", post_id);
                        if (listExpands.contains(post_id)) {
                ...
                case R.id.tv_creator_name:
                    Logger.e("----->跳转用户信息");
                    if (itemData != null) {
                        NearbyPostItemEntity postItemEntity = itemData.getItem();
                        if (postItemEntity != null) {
                            NearbyPostItemEntity.MemberEntity memberEntity = postItemEntity.getMembers();
        ...
        public class ItemData implements Serializable {
            ...
            private NearbyPostItemEntity item;
        }

    }

    //加入社区
    private void joinCommunity(String community_id) {
        Map<String, String> param = new HashMap<>();
        param.put("mid", PaperUtils.getMId());
        param.put("session_key", PaperUtils.getSessionKey());
                ...
                if (shareInfoEntity != null) {
                    ...
                    if (code == Constants.CODE_SUCCESS) {
                        ...
                        if (dataEntity != null) {
                            ...
                            umWeb.setThumb(new UMImage(NearbyPostActivity.this, img));
    ...
    //跳转用户信息
    private void goMemberInfo(NearbyPostItemEntity.MemberEntity memberEntity) {
    ...
    private void collectAdvertising(final PostAdapter.ItemData itemData) {
        ...
        if (postItemEntity != null) {
            //来源
            String community_id = "";
            List<NearbyPostItemEntity.CommunityEntity> community_list = postItemEntity.getCommunity_list();
            if (community_list != null && community_list.size() > 0) {
                NearbyPostItemEntity.CommunityEntity communityEntity = community_list.get(0);
                if (communityEntity != null) {
                    community_id = communityEntity.getId();
                }
            }
    ...
    private class PostMediaAdapter extends BaseQuickAdapter<NearbyPostItemEntity.AttachmentEntity, BaseViewHolder> {

        private List<NearbyPostItemEntity.AttachmentEntity> data = new ArrayList<>();
        private ArrayList<ThumbViewInfo> listImg = new ArrayList<>();

        public PostMediaAdapter(@LayoutRes int layoutResId, @Nullable List<NearbyPostItemEntity.AttachmentEntity> data) {
            super(layoutResId, data);
            this.data = data;

            for (int i = 0; i < data.size(); i++) {
                NearbyPostItemEntity.AttachmentEntity entity = data.get(i);
                if (entity != null) {
        ...
        @Override
        protected void convert(final BaseViewHolder helper, NearbyPostItemEntity.AttachmentEntity item) {
